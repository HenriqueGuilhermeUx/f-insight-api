...
async function refreshMacroData() {
  ...
  const indicators = [
    indicatorFromResult('selic', selicResult, 'Principal referência para custo do dinheiro, crédito, taxa de desconto e comparação entre fluxos financeiros.', classifySelic),
    indicatorFromResult('ipca', ipcaResult, 'Mede inflação ao consumidor e ajuda a contextualizar poder de compra, custos e expectativas de juros.', classifyIpca),
    indicatorFromResult('usdbrl', usdResult, 'Ajuda a contextualizar exposição cambial, custos dolarizados, receitas externas e inflação.', classifyUsd)
  ].filter(Boolean);
